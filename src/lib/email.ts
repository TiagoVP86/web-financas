...
export async function sendVerificationEmail(to: string, name: string, token: string) {
  ...
  const from = process.env.EMAIL_FROM ?? `Finanças+ <${process.env.GMAIL_USER}>`
  ...
  try {
    ...
    await transporter.sendMail({
      from,
      to,
      subject: "Confirme seu cadastro no Finanças+",
      html: `<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8" />
...
<body style="margin:0;padding:0;background:#f5f5f1;font-family:Inter,system-ui,sans-serif;">
    ...
    <tr>
      <td align="center">
        <table width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e5e5e5;">
          <tr>
            <td style="background:#047857;padding:28px 40px;">
              <p style="margin:0;font-size:18px;font-weight:700;color:#ffffff;letter-spacing:-0.01em;">Finanças+</p>
            </td>
          </tr>
          <tr>
            <td style="padding:40px;">
              ...
              <a href="${url}" style="display:inline-block;background:#059669;color:#ffffff;font-size:14px;font-weight:600;text-decoration:none;padding:12px 28px;border-radius:8px;">
                Confirmar email
              </a>
